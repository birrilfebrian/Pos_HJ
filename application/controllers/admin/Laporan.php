...
class Laporan extends CI_Controller{
	...
	if($this->session->userdata('akses')=='1'){
		...
		$data['jual_hari']=$this->m_laporan->get_hari_jual();
	...
	function lap_laba_rugi_tahunan(){
		$tahun=$this->input->post('thn');
		$x['jml']=$this->m_laporan->get_total_lap_laba_rugi_tahunan($tahun);
		$x['data']=$this->m_laporan->get_lap_laba_rugi_tahunan($tahun);
		$this->load->view('admin/laporan/v_lap_laba_rugi',$x);
	}
	function lap_laba_rugi_harian(){
		$hari=$this->input->post('hari');
		$x['jml']=$this->m_laporan->get_total_lap_laba_rugi_harian($hari);
		$x['data']=$this->m_laporan->get_lap_laba_rugi_harian($hari);
		$this->load->view('admin/laporan/v_lap_laba_rugi',$x);
	}
}